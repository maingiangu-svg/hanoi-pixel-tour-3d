...
export class Game {
  constructor(container, uiRoot) {
    ...
    this.storyManager = new StoryManager({
      ...
      ui: this.ui,
      clock: this.clock,
      onChapterComplete: (chapterId, nextChapterId) => {
        this.saveSystem?.saveSoon()
      },
    })
    ...
    this.collectibles = new CollectibleManager({
      ...
      playerPosition: this.player.camera.position,
      ui: this.ui,
      onCollect: (item, stats) => {
        this.saveSystem?.saveSoon()
        if (stats.collected === stats.total) {
          this.ui.showNotice?.('🏆 Thu thập hoàn tất! Bạn là bậc thầy khám phá Hà Nội!', 5000)
        }
      },
    })
  ...
  tick() {
    ...
    this.weather?.applyToScene(this.renderer.scene)
